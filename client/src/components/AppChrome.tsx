...
/**
 * Remaining render credit, in the sidebar.
 *
 * The balance has always been on the user record but nothing displayed it, so the first
 * a customer knew about running out was a red error at the moment they hit Approve.
 */
function CreditPill() {
  const { locale } = useLocale();
  const t = copy[locale].billing;
  const [, setLocation] = useLocation();
  const overview = trpc.billing.overview.useQuery(undefined, { refetchOnWindowFocus: true });
  if (!overview.data) return null;

  const total = overview.data.videos.total;
  const empty = total === 0;
  return (
    <button
      onClick={() => setLocation("/billing")}
      className={`mt-4 w-full rounded-2xl border px-3 py-3 text-start ${empty ? "border-[#C2643C]/30 bg-[#FFEFE5]" : "border-[#251811]/8 bg-white/60"}`}
    >
      <p className={`text-lg font-bold ${empty ? "text-[#94522C]" : "text-[#34241B]"}`}>{total}</p>
      <p className="text-xs text-[#7D736D]">{empty ? t.outOfCredit : `${t.videosLabel} · ${t.creditsLeft}`}</p>
    </button>
  );
}
