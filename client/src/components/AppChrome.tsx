...
export function AppSidebar({ children }: { children: ReactNode }) {
  ...
  if (!user && error) return <div className="grid min-h-screen place-items-center bg-[#F7F5EF] p-5"><div className="w-full max-w-md rounded-[28px] border border-[#11251E]/10 bg-white p-8 text-center shadow-[0_24px_60px_rgba(17,37,30,.11)]"><div className="mx-auto mb-6 w-fit"><Brand /></div><h1 className="serif text-3xl text-[#11251E]">{t.common.errorTitle}</h1><p className="mt-3 text-sm leading-6 text-[#68766e]">{t.common.errorBody}</p><p className="mt-4 rounded-xl bg-[#FFF1E5] px-4 py-3 text-xs leading-5 text-[#94572C]">{error.message}</p><button onClick={() => void refresh()} className="mt-7 inline-flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#11251E] text-sm font-bold text-white">{t.common.retry}</button></div></div>;
  if (!user) return <div className="grid min-h-screen place-items-center bg-[#F7F5EF] p-5"><div className="w-full max-w-md rounded-[28px] border border-[#11251E]/10 bg-white p-8 text-center shadow-[0_24px_60px_rgba(17,37,30,.11)]"><div className="mx-auto mb-6 w-fit"><Brand /></div><h1 className="serif text-3xl text-[#11251E]">{t.common.signInTitle}</h1><p className="mt-3 text-sm leading-6 text-[#68766e]">{t.common.signInBody}</p><button onClick={() => setLocation("/auth")} className="mt-7 inline-flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#11251E] text-sm font-bold text-white">{t.common.continue}<ArrowUpRight size={16}/></button></div></div>;
