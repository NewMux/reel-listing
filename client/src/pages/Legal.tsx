...
const sections = [
  ["Using the service", "VistaFlow AI provides tools for creating real estate video projects. You are responsible for ensuring that you have the necessary rights, permissions, and consents for all property media, listing information, and brand materials you submit."],
...
export default function Legal({ type }: { type: "terms" | "privacy" }) {
  ...
  return <div className="min-h-screen bg-[#F7F5EF] text-[#11251E]"><PublicNav/><main className="mx-auto max-w-3xl px-5 pb-24 pt-16 sm:px-8 sm:pt-24"><p className="text-xs font-bold uppercase tracking-[.15em] text-[#70834C]">VistaFlow AI</p><h1 className="serif mt-4 text-5xl tracking-[-.05em] sm:text-6xl">{title}</h1><p className="mt-4 text-sm text-[#718077]">{t.updated}</p><p className="mt-10 border-s-2 border-[#B7CF77] ps-5 text-lg leading-8 text-[#405448]">{intro}</p><div className="mt-14 space-y-10">{relevant.map(([heading, body]) => <section key={heading}><h2 className="text-lg font-bold text-[#1E382B]">{heading}</h2><p className="mt-3 text-sm leading-7 text-[#607168]">{body}</p></section>)}</div></main><Footer/></div>;
}
